...
contract FIPCOINStaking is Initializable, OwnableUpgradeable, ReentrancyGuardUpgradeable, PausableUpgradeable, UUPSUpgradeable {
    ...
    uint256 public LOCK_PERIOD_60_DAYS;
    uint256 public LOCK_PERIOD_90_DAYS;
    uint256 public LOCK_PERIOD_180_DAYS;

    uint256 public REWARD_RATE_60_DAYS;
    uint256 public REWARD_RATE_90_DAYS;
    uint256 public REWARD_RATE_180_DAYS;
...
function initialize(IERC20Upgradeable _Fipcoin, address feeTake, address _usdt) public initializer {
    ...
    LOCK_PERIOD_60_DAYS = 60 * 24 * 3600;
    LOCK_PERIOD_90_DAYS = 90 * 24 * 3600;
    LOCK_PERIOD_180_DAYS = 180 * 24 * 3600;

    REWARD_RATE_60_DAYS = 4;
    REWARD_RATE_90_DAYS = 7;
    REWARD_RATE_180_DAYS = 16;
...
function updateFipcoin(address _fipcoin) external onlyOwner{
    Fipcoin = IERC20Upgradeable(_fipcoin);
     ...
     function updateLockPeriods(
        uint256 _lockPeriod60Days,
        uint256 _lockPeriod90Days,
        uint256 _lockPeriod180Days
    ) external onlyOwner {
        LOCK_PERIOD_60_DAYS = _lockPeriod60Days;
        LOCK_PERIOD_90_DAYS = _lockPeriod90Days;
        LOCK_PERIOD_180_DAYS = _lockPeriod180Days;
    }

    // Update reward rates
    function updateRewardRates(
        uint256 _rewardRate60Days,
        uint256 _rewardRate90Days,
        uint256 _rewardRate180Days
    ) external onlyOwner {
        REWARD_RATE_60_DAYS = _rewardRate60Days;
        REWARD_RATE_90_DAYS = _rewardRate90Days;
        REWARD_RATE_180_DAYS = _rewardRate180Days;
    }
 ...
 function buyToken(uint256 usdtAmount) external {
    ...
    function stake(uint256 _amount, uint256 _lockPeriod) external nonReentrant whenNotPaused {
        ...
        require(
            _lockPeriod == LOCK_PERIOD_60_DAYS || _lockPeriod == LOCK_PERIOD_90_DAYS || _lockPeriod == LOCK_PERIOD_180_DAYS,
            "Invalid lock period"
        );
  ...
  function calculateRewards(address _staker) internal view returns (uint256) {
    ...
    if (staker.lockPeriod == LOCK_PERIOD_60_DAYS) {
        rewardRate = REWARD_RATE_60_DAYS;  // 4% total over 60 days
        lockDurationInHours = 60 * 24;     // 60 days * 24 hours = 1440 hours
    } else if (staker.lockPeriod == LOCK_PERIOD_90_DAYS) {
        rewardRate = REWARD_RATE_90_DAYS;  // 7% total over 90 days
        lockDurationInHours = 90 * 24;     // 90 days * 24 hours = 2160 hours
    } else {
        rewardRate = REWARD_RATE_180_DAYS; // 16% total over 180 days
        lockDurationInHours = 180 * 24;    // 180 days * 24 hours = 4320 hours
    }

    uint256 hourlyRewardRate = (rewardRate * 1e18) / lockDurationInHours;
